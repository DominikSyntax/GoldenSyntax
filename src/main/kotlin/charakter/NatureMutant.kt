  ...
  \ \ \/\ \L\ \ \ \_/ \_/ \/\  __/\ \ \/      /\ \L\ \ \ \_/     \ \ \_\ \ \ \ \/\  __/    \ \ \`\ \/\ \L\.\_\ \ \_\ \ \_\ \ \ \//\  __/ 
   \ \_\ \____/\ \___x___/'\ \____\\ \_\      \ \____/\ \_\       \ \__\\ \_\ \_\ \____\    \ \_\ \_\ \__/.\_\\ \__\\ \____/\ \_\\ \____\
    ...
    fun thunderStorm(enemies: MutableList<Enemy>) {
        println("Es ziehen dunkle Wolken über dem Schlachtfeld auf ...")
        for (enemy in enemies) {
            if (enemy.healthPower > 0) {
                var shocksInt = (1..3).random()
                var abzug = (25..50).random()
                abzug == (abzug / 100 * damagePower).toInt()
                ...
                repeat(shocksInt) {
                    if (enemy.healthPower >= abzug) {
                        ...
                        println()
                        println(
                            "Das war ein Blitz zuviel für ${enemy.name}. Die Spannung war so stark, das das Blut angefangen hat zu kochen und der ganze Körper, \n" +
                                    "wie eine heiße Fleischwurst in der Microwelle geplatzt ist"
                        )
                        enemy.healthPower = 0
                        enemy.isDead = true
                    }
